...
class EverestEvaluatorStep(StepMixin, DefaultEnsembleEvaluatorComputeStep):
...
def create_evaluator(evaluator: Evaluator) -> EverestEvaluatorStep:
    """Create an ensemble evaluator.

    Invoking this function returns an
    [`EverestEvaluatorStep`][ropt_everest.EverestEvaluatorStep] object, which
    you can execute using its [`run`][ropt_everest.EverestEvaluatorStep.run]
    method.

    Returns:
        An `EverestEvaluatorStep` object, representing the added evaluator.
    """
    step = create_compute_step("everest/evaluator", evaluator=evaluator)
    assert isinstance(step, EverestEvaluatorStep)
    return step
