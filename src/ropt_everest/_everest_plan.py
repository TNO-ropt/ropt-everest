...
class EverestPlan:
    ...
    **Key Features:**
    ...
    -   **Arbitrary Value Storage:** The `EverestPlan` object can store and
        retrieve arbitrary values using the indexing operator (`[]`). This
        allows you to dynamically add or modify data that can be used by any
        step of the plan. This is especially useful when you need to change the
        metadata during a sequence of executions, as metadata can reference
        these values.

    **Example of Arbitrary Value Storage:**
    ...
    **Example of Dynamic Metadata:**
    ...
    def add_optimizer(
        ...
        configuration dictionary. Invoking this method returns an
        ...
        **Metadata**:
        ...
        - The keys in the `metadata` dictionary are used as column names in
          the output tables.
        - Dynamic Metadata: If a metadata value is a string that begins with a
          dollar sign (`$`), it's treated as a dynamic reference. The portion of
          the string following the `$` is used as a key to look up a value
          within the execution plan itself (accessed using `plan[key]`). This
          allows you to modify the metadata between runs by changing values in
          the plan.
    ...
    def add_evaluator(
        ...
        You can tailor the evaluator's behavior by providing an optional
        configuration dictionary. Invoking this method returns an
        ...
        **Metadata**:
        ...
        - The `metadata` parameter allows you to associate arbitrary data with
          each result generated by the evaluator.
        - This metadata is included in generated tables and data frames.
        - The keys in the `metadata` dictionary are used as column names in the
          output tables.
        - Dynamic Metadata: If a metadata value is a string that begins with a
          dollar sign (`$`), it's treated as a dynamic reference. The portion of
          the string following the `$` is used as a key to look up a value
        ...
        ```python
        evaluator = plan.add_evaluator(metadata={"run_id": "$run_number"})
        # ... later in your code ...
        plan["run_number"] = 5
        ...
        Args:
            config:   An optional dictionary containing the Everest configuration
                     for the evaluator. If omitted, the default configuration is
                      used.
            metadata: An optional dictionary of metadata to associate with the
                      evaluator's results.
...
class EverestOptimizerStep(EverestStep):
    ...
    def run(self, variables: ArrayLike | None = None) -> OptimizerExitCode:
        ...
        This method executes the underlying optimizer with the given variables.
        If no variables are provided, the optimizer will use its default
        initialization.
...
class EverestEvaluatorStep(EverestStep):
    ...
    def run(self, variables: ArrayLike | None = None) -> OptimizerExitCode:
        ...
        vectors, or a 2D matrix where the variable vectors are the rows.

        Args:
            variables: An array-like object containing the variables for the
                       optimization.
